// Failas: js/logger.js (Versija V1.1.5 - Savarankiška Supabase iniciacija)

(function() {
    'use strict';

    // === SVARBU: Čia patys sukuriame kliento versiją, kad išvengtume konfliktų ===
    const SUPABASE_URL = 'https://zojhurhwmceoqxkatvkx.supabase.co';
    const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inpvamh1cmh3bWNlb3F4a2F0dmt4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDkxNjYxNDYsImV4cCI6MjA2NDc0MjE0Nn0.NFGhQc7H95U9vOaM7OVxNUgTuXSughz8ZuxaCLfbfQE';
    
    // Sukuriame vietinį klientą, kuris tikrai veiks
    ...
    async function handleCreate() {
        ...
        if (operations.length === 0) throw new Error("Neįvesta jokia suma arba suma yra nulinė.");
        for (const op of operations) await createSingleLogEntry(op);
    }

    async function createSingleLogEntry(entryData) {
        // === NAUDOJAME VIETINĮ KLIENTĄ 'sb' ===
        const { data: { user } } = await sb.auth.getUser();
        if (!user) throw new Error("Vartotojas neprisijungęs. Pabandykite prisijungti iš naujo.");
        ...
        if (window.appActions && window.appActions.getPriceForDate) {
            ...
            } catch (e) { console.warn("Nepavyko gauti kainos:", e); }
        }
    ...
    function handleAccordionToggle(separatorRow) {
        ...
        transactionRows.forEach(row => { row.classList.toggle('hidden'); });
    }
    ...
    function updateVisibleFields() {
        ...
        if (platform === 'go' && (category === 'Gem Removal' || category === 'Socket Unlock')) {
            ...
            if (platform === 'go') tokensForPlatform = ['ggt', 'gmt', 'usdc'];
            else if (platform === 'og') tokensForPlatform = ['gst', 'gmt', 'sol', 'usdc'];
            updateTokenRadioButtons(tokensForPlatform);
        }
    }
